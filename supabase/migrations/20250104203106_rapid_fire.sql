/*
  ...
  1. Changes
    ...
    - Ensure proper data type for telegram_user_id
...
BEGIN
  ...
  -- Modify column type to bigint with USING clause
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'orders' AND column_name = 'telegram_user_id'
  ) THEN
    ALTER TABLE orders 
    ALTER COLUMN telegram_user_id TYPE bigint USING telegram_user_id::text::bigint;
  END IF;
